...
export function ToolModuleView({ module }: { module: ModuleDefinition }) {
  ...
  const compact = ["reports", "terminal"].includes(module.slug);
  return (
    <div className="module-view">
      <header className="page-heading"><span className="page-heading__icon"><Icon name={module.icon} size={24} /></span><span>{!compact ? <small>PROJECT TOOLS</small> : null}<h1>{module.label}</h1>{!compact ? <p>{module.description}</p> : null}</span><span className="shell-status"><span className="live-dot" />Phase 2 backend</span></header>
...
function ToolGrid({ stats = [], children }: { stats?: Array<[string, string]>; children: React.ReactNode }) {
  return <>{stats.length ? <div className="module-stat-strip">{stats.map(([label, value]) => <span key={label}><strong>{value}</strong><small>{label}</small></span>)}</div> : null}<section className="module-layout original-module-layout tool-module-layout">{children}</section></>;
}